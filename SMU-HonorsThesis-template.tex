...
A fundamental and core building block of general relativity is the tensor. Tensors are mathematical objects that behave much like matrices but obey a strict set of transformation laws. Tensors exists in $m$-dimensional space and every tensor has a rank. The rank, $n$, od a tensor is dependent on the number of indices it has. For example, a tensor of rank $n$ in $m$-dimensional space has $m^{n}$ components distributed across $n$ indices. It is conventional to use 1, 2, 3, ... as the iterators across each of the indices, representing the different spatial dimensions. When working with space-time it is convention to reserve the $0^{th}$ iteration on each index as the time dimension. For example, the origin of a 3 dimensional space would normally be represented by a vector as $\xshlonghvecr{O}=0\hat{i}+0\hat{j}+0\hat{k}$ or $\langle 0, 0, 0\rangle$. This very same origin can be represented in tensor form as $\textup{O}^{\alpha}$ where $\alpha=1, 2, 3$ and $\textup{O}^{1}=0, \textup{O}^{2}=0, \textup{O}^{3}=0$. When the entire tensor needs to be seen it is represented enclosed in brackets. For a tensor of rank $n$, the tensor then has $n$ rows, for example a rank 3 tensor $\textup{A}^{\alpha \beta \gamma}$ would be.
